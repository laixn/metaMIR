# companion file to metaMIR.R that performs the analytical functions
# parameters for execution are explained in the calling script file

# check flag of whether script is run as command line or not
if (comline){
...
batch_metaMIR <- function(inputfile, mincomb=5, maxclust=15, Zthresh=1, outPath=NA){
  # print the run status if the script is running from command line
  if (comline) print(paste("input=",inputfile,mincomb,maxclust,Zthresh,outPath));

  # check supplied parameters. Input file is absolutely needed
  if (missing(inputfile)){
    stop("Missing input file of genes to analyze.")
  }
  # provide a warning if the score threshold is low
  if(Zthresh < 0.75){
    warning("Zthreshold < 1 - results may be noisy.")
  }
  # fitting data were only generated up to genelists of length 25
  # and the standardization fitting does not model the data below
  # a cluster size of 4. Ensure mincomb and maxclust are withing an
  # analyzable range
  if(maxclust > 25){
  ...
  # function arguments are a single row matrix of one miRNA with column names
  # corresponding to gene names. Values are the predicted scores of interaction
  # between the miRNA and each gene. If no predicted interaction, the value
  # is NA and these are removed. Combinations are generated among the
  # genes for which there is a predicted interaction.
  xtrgm <- function(x,mincomb){
    x <- x[1,!is.na(x),drop=F]
    # if number of non-missing scores is less than the minimum combination
    # return NULL and exit the subroutine
    if (length(x) < mincomb) return(NULL)
    # generate combinations among the genes predicted
    # to be targeted by the miRNA
    if (length(x) == mincomb){
  ...
  # source subroutines necessary for score calulation and matrix creation
  ...
  # clusters can be written to files by uncommenting write statements
  partgenes <- function(inputgenes, anatype,mat){
    ...
    # calculate the default Euclidean distance matrix using contingency
    # matrix
    dcm <- dist(cm2)
    # calculate the adjacency matrix, taking the magnitude of the dot
    # product of the two gene vectors. Returns a 1 where both genes are
    # targeted by the same miRNA, 0 otherwise
    ...
    # convert adjusted distance matrix to long format
...
#       write.table(dcmout,file=fname1,col.names=T,row.names=F,quote=F,sep="\t")
  ...
  # core metaMIR function that calculates a score for miRNAs
  # by gene cluster. Arguments:
  # inputgenes: list of genes to analyze
  # state: is analysis positive or negative?
  # mincomb: the minimum combination size to analyze
  ...
  # define a matrix with flags to track the analysis status for each
  # input gene.
  # kern: kernel, genes prefixed with an asterisk
  # pos: genes in the default, positive analysis (identify miRNAs targeting)
  # neg: genes in the negative analysis (not to be targeted by miRNAs)
  genelist.mat <- data.table(src=genelist.raw,kern=NA,pos=NA,neg=NA)
  # deconstruct input string for each gene, checking for * or - prefixes
  ...
  # check length of analysis gene list. If shorter than the specified
  # maximum cluster size (maxclust) reset this value
  if (length(genelist.src)<maxclust) maxclust <- length(genelist.src)
  # check that there is prediction information for genes entered. If not
  # exit script and return NULL
  ...
  # define the 'kernel' if there were asterisked genes that need to
  # appear in returned combinations
